...
class FTPServer:
    ...
    def start(self):
        ...
        if not self._run:
            ...
            loop.create_task(asyncio.start_server(self.server, "0.0.0.0", self.port))
    ...
    async def USER(self, stream, argument):
        ...
        await _awrite(stream, "230 Logged in.\r\n")
        return True

    async def PASS(self, stream, argument):
        await _awrite(stream, "230 Passwd Ok.\r\n")
        return True

    async def SYST(self, stream, argument):
        await _awrite(stream, "215 UNIX Type: L8\r\n")
        return True

    async def NOOP(self, stream, argument):
        await _awrite(stream, "200 OK\r\n")
        return True

    async def FEAT(self, stream, argument):
        await _awrite(stream, "211 no-features\r\n")
        return True
    ...
    async def TYPE(self, stream, argument):
        ...
        await _awrite(stream, "200 Binary mode.\r\n")
        return True
    ...
    async def RETR(self, stream, argument):
        ...
        await _awrite(stream, "150 Opening data connection\r\n")
        ...
        if self.transfer_rpl:
            await _awrite(stream, self.transfer_rpl)
            self.transfer_rpl = False
        else:
            await _awrite(stream, "550 File Load Error.\r\n")
    ...
    async def STOR(self, stream, argument):
        ...
        if self.transfer_rpl:
            ...
            await _awrite(stream, "550 File Save Error.\r\n")
    ...
    async def LIST(self, stream, argument):
        ...
        await _awrite(stream, "150 Here comes the directory listing.\r\n")
        self.transfer = "LIST"
        await self.send_data(self.con_type)
        await _awrite(stream, "226 Directory send okey.\r\n")
        return True
