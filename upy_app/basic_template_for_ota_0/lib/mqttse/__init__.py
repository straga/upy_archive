...
class MQTTClient:
    ...
    def __init__(self, client_id, server=None, port=1883, user=None, password=None, keepalive=0, response_time=1, sbt=None,
        ping_interval = 0):
    ...
    async def pub_bus(self):
        ...
        if self.mqtt_bus and self.status == 1:
            ...
            for key, value in list(self.mqtt_bus.items()):

                if key in self.topics and value:
    ...
    async def _keep_alive(self):
        ...
        while True:
            if self.status == 1:
                self.to_mqtt_bus('ping', 1)
            await asyncio.sleep(30)


    def to_mqtt_bus(self, topic, msg, retain=False):
        self.mqtt_bus[topic] = {"msg": msg, "retain": retain}


    #mqtt protocol ping
    # async def _ping(self):
    #
    #     await self.as_write(b"\xc0\0")
    #
    # async def _keep_alive(self):
    #
    #     while True:
